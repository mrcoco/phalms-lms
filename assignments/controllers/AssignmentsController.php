...
use Modules\Assignments\Models\Assignments;
...
class AssignmentsController extends ControllerBase
{
    ...
    public function listAction()
    {
        ...
        $qryTotal = Assignments::find($arProp);
    ...
    {
        $this->view->disable();
        $data = Assignments::findFirst($this->request->getPost('hidden_id'));
         $data->teacher_id = $this->request->getPost('teacher_id');
	 $data->title = $this->request->getPost('title');
	 $data->file = $this->request->getPost('file');
	 $data->content = $this->request->getPost('content');
    ...
    {
        $data = Assignments::findFirst($this->request->getQuery('id'));
        $response = new \Phalcon\Http\Response();
        $response->setContentType('application/json', 'UTF-8');
    ...
    {
        $this->view->disable();
        $data   = Assignments::findFirstById($id);

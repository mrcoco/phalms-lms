...
class GradeController extends ControllerBase
{
    public function initialize()
    {
        $this->assets
            ->collection('footer')
            ->setTargetPath("themes/admin/assets/js/combined-grade.js")
            ->setTargetUri("themes/admin/assets/js/combined-grade.js")
            ->join(true)
            ->addJs($this->config->modules->lms."grade/views/js/js.js")
    ...
    {
        $this->view->disable();
        $data = Grade::findFirst($this->request->getPost('hidden_id'));
        $data->name = $this->request->getPost('name');
	    $data->description = $this->request->getPost('description');
    ...
    {
        $data = Grade::findFirst($this->request->getQuery('id'));
        $response = new \Phalcon\Http\Response();
        $response->setContentType('application/json', 'UTF-8');

...
use Modules\Discussions\Models\Discussions;
...
class DiscussionsController extends ControllerBase
{
    public function initialize()
    {
        $this->assets
            ->collection('footer')
            ->setTargetPath("themes/admin/assets/js/combined-discussions.js")
            ->setTargetUri("themes/admin/assets/js/combined-discussions.js")
            ->join(true)
            ->addJs($this->config->modules->lms."discussions/views/js/js.js")
    ...
    public function listAction()
    {
        ...
        $qryTotal = Discussions::find($arProp);
    ...
    public function createAction()
    {
        ...
        $data = new Discussions();
         $data->parrent_id = $this->request->getPost('parrent_id');
    ...
    {
        $this->view->disable();
        $data = Discussions::findFirst($this->request->getPost('hidden_id'));
         $data->parrent_id = $this->request->getPost('parrent_id');
	 $data->classroom_id = $this->request->getPost('classroom_id');
	 $data->user_id = $this->request->getPost('user_id');
	 $data->content = $this->request->getPost('content');
    ...
    {
        $data = Discussions::findFirst($this->request->getQuery('id'));
        $response = new \Phalcon\Http\Response();
        $response->setContentType('application/json', 'UTF-8');
    ...
    {
        $this->view->disable();
        $data   = Discussions::findFirstById($id);
